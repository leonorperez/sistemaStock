...
public class JWTAuthorizationFilter extends OncePerRequestFilter {
    ...
    private boolean isValidUrl(HttpServletRequest request) {
        ...
        if (method.equals("GET")) {
            ...
            return requestURI.matches("/api/user/\\w+-\\w+-\\w+-\\w+-\\w+") || requestURI.matches("/api/users")|| requestURI.matches("/api/product/\\w+-\\w+-\\w+-\\w+-\\w+") || requestURI.matches("/api/products");
        } else if (method.equals("POST")) {
            return requestURI.equals("/api/user/new") || requestURI.equals("/api/login")|| requestURI.equals("/api/product/new");
        } else if (method.equals("PUT") || method.equals("DELETE")) {
            // Permitir URLs que comiencen con "/api/user/" y tengan un identificador UUID al final
            return requestURI.matches("/api/user/\\w+-\\w+-\\w+-\\w+-\\w+") || requestURI.matches("/api/user/\\w+-\\w+-\\w+-\\w+-\\w+/.*")|| requestURI.matches("/api/product/\\w+-\\w+-\\w+-\\w+-\\w+")|| requestURI.matches("/api/product/\\w+-\\w+-\\w+-\\w+-\\w+/.*");
        }
